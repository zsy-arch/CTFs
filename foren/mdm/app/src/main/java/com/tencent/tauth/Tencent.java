...
public class Tencent {
    ...
    public static synchronized Tencent createInstance(String str, Context context) {
        ...
        synchronized (Tencent.class) {
            ...
            f.c(TAG, "createInstance()  -- start, appId = " + str);
            if (sInstance == null) {
            ...
            if (!checkManifestConfig(context, str)) {
                ...
                OpenConfig.getInstance(context, str);
    ...
    public static boolean onActivityResultData(int i, int i2, Intent intent, IUiListener iUiListener) {
        boolean z = true;
        StringBuilder append = new StringBuilder().append("onActivityResultData() reqcode = ").append(i).append(", resultcode = ").append(i2).append(", data = null ? ").append(intent == null).append(", listener = null ? ");
        if (iUiListener != null) {
            z = false;
        }
        f.c(TAG, append.append(z).toString());
    ...
    public void handleLoginData(Intent intent, IUiListener iUiListener) {
        boolean z = true;
        StringBuilder append = new StringBuilder().append("handleLoginData() data = null ? ").append(intent == null).append(", listener = null ? ");
        if (iUiListener != null) {
            z = false;
        }
        f.c(TAG, append.append(z).toString());
    ...
    public static void handleResultData(Intent intent, IUiListener iUiListener) {
        boolean z = true;
        StringBuilder append = new StringBuilder().append("handleResultData() data = null ? ").append(intent == null).append(", listener = null ? ");
        if (iUiListener != null) {
            z = false;
        }
        f.c(TAG, append.append(z).toString());
    ...
    public boolean isSupportSSOLogin(Activity activity) {
        return (Util.isTablet(activity) && SystemUtils.getAppVersionName(activity, Constants.PACKAGE_QQ_PAD) != null) || SystemUtils.compareQQVersion(activity, "4.1") >= 0 || SystemUtils.compareTimVersion(activity, "1.1") >= 0;
    }
