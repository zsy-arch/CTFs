...
final class DecodedBitStreamParser {
    ...
    public static DecoderResult decode(byte[] bytes, int mode) {
        ...
        switch (mode) {
            ...
            case 3:
                if (mode == 2) {
                    postcode = new DecimalFormat("0000000000".substring(0, getPostCode2Length(bytes))).format(getPostCode2(bytes));
                } else {
                ...
                if (!result.toString().startsWith("[)>\u001e01\u001d")) {
                    result.insert(0, String.valueOf(postcode) + GS + country + GS + service + GS);
                    break;
                } else {
                    result.insert(9, String.valueOf(postcode) + GS + country + GS + service + GS);
                    break;
                }
            case 4:
    ...
    private static int getPostCode2(byte[] bytes) {
        return getInt(bytes, new byte[]{33, 34, 35, 36, 25, JSONLexer.EOI, 27, 28, 29, 30, 19, 20, 21, 22, 23, 24, dc.k, dc.l, dc.m, dc.n, 17, 18, 7, 8, 9, 10, 11, 12, 1, 2});
    }

    private static String getPostCode3(byte[] bytes) {
        return String.valueOf(new char[]{SETS[0].charAt(getInt(bytes, new byte[]{39, 40, 41, 42, 31, 32})), SETS[0].charAt(getInt(bytes, new byte[]{33, 34, 35, 36, 25, JSONLexer.EOI})), SETS[0].charAt(getInt(bytes, new byte[]{27, 28, 29, 30, 19, 20})), SETS[0].charAt(getInt(bytes, new byte[]{21, 22, 23, 24, dc.k, dc.l})), SETS[0].charAt(getInt(bytes, new byte[]{dc.m, dc.n, 17, 18, 7, 8})), SETS[0].charAt(getInt(bytes, new byte[]{9, 10, 11, 12, 1, 2}))});
    }
        ...
        while (i < start + len) {
            ...
            switch (c) {
                ...
                case 65531:
                    ...
                    int i5 = i4 + 1;
                    i = i5 + 1;
                    sb.append(NINE_DIGITS.format((bytes[i2] << 24) + (bytes[i3] << 18) + (bytes[i4] << 12) + (bytes[i5] << 6) + bytes[i]));
                    shift = shift2;
                    break;
            }
            shift2 = shift - 1;
