    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints) throws NotFoundException {
        ...
        if (nextStart >= this.counterLength || trailingWhitespace >= lastPatternSize / 2) {
            ...
            if (!arrayContains(STARTEND_ENCODING, this.decodeRowResult.charAt(0))) {
                throw NotFoundException.getNotFoundInstance();
            } else if (!arrayContains(STARTEND_ENCODING, this.decodeRowResult.charAt(this.decodeRowResult.length() - 1))) {
                throw NotFoundException.getNotFoundInstance();
                ...
                return new Result(this.decodeRowResult.toString(), null, new ResultPoint[]{new ResultPoint(left, rowNumber), new ResultPoint(runningCount, rowNumber)}, BarcodeFormat.CODABAR);
            }
        } else {
            throw NotFoundException.getNotFoundInstance();
        }
