        ...
        synchronized (this) {
            ...
            if (result == Permissions.GRANTED) {
                if (this.mPermissions.isEmpty()) {
                    new Handler(this.mLooper).post(new Runnable() { // from class: com.cdlinglu.utils.permissions.PermissionsResultAction.1
                ...
                new Handler(this.mLooper).post(new Runnable() { // from class: com.cdlinglu.utils.permissions.PermissionsResultAction.2
                    @Override // java.lang.Runnable
                    public void run() {
                ...
                if (result == Permissions.NOT_FOUND) {
                    if (!shouldIgnorePermissionNotFound(permission)) {
                        new Handler(this.mLooper).post(new Runnable() { // from class: com.cdlinglu.utils.permissions.PermissionsResultAction.4
                            @Override // java.lang.Runnable
                            public void run() {
                                PermissionsResultAction.this.onDenied(permission);
                            }
                        ...
                        new Handler(this.mLooper).post(new Runnable() { // from class: com.cdlinglu.utils.permissions.PermissionsResultAction.3
                            @Override // java.lang.Runnable
                            public void run() {
                                PermissionsResultAction.this.onGranted();
                            }
