...
public abstract class BaseActivity extends AppCompatActivity implements View.OnClickListener, IBaseActivity {
    ...
    private void init() {
        ...
        try {
            ...
            if (layout >= 1) {
                ...
                if (!custumHeader && this.flyMain != null) {
    ...
    protected void showNavigation(int drawId) {
        if (drawId > 0) {
            this.toolbar.setNavigationIcon(drawId);
        } else {
    ...
    protected void setHeaderRightTxt(@StringRes int right) {
        if (right > 0) {
            setHeaderRightTxt(getString(right));
        }
    }

    protected void setHeaderRightTxt(CharSequence right) {
        if (this.toolbar.findViewById(R.id.head_vRight) == null) {
    ...
    protected void setHeaderRightEnabled(boolean right) {
        if (this.toolbar.findViewById(R.id.head_vRight) != null) {
            ((TextView) getView(this.toolbar, R.id.head_vRight)).setEnabled(right);
        }
    }

    protected void addHeaderRight(@DrawableRes int right, @IdRes int id) {
        this.rightCount++;
        View v = View.inflate(this.context, R.layout.in_head_right, null);
        ImageView img = (ImageView) getView(v, R.id.head_vRight);
        img.setId(id);
    ...
    protected void startActClear(Class<?> cls) {
        startActClear((Intent) null, cls);
    }

    @Deprecated
    protected void startActClear(Class<?> cls, Bundle bundle) {
        if (this.app != null) {
            this.app.clear();
        }
        startAct(cls, bundle);
    ...
    protected Bundle getBundle() {
        return getIntent().hasExtra(Constant.BUNDLE) ? getIntent().getBundleExtra(Constant.BUNDLE) : new Bundle();
    }
