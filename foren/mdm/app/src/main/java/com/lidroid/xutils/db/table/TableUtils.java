...
public class TableUtils {
    ...
    public static String getTableName(Class<?> entityType) {
        ...
        return (table == null || TextUtils.isEmpty(table.name())) ? entityType.getName().replace('.', '_') : table.name();
    }
    ...
    public static synchronized HashMap<String, Column> getColumnMap(Class<?> entityType) {
        ...
        synchronized (TableUtils.class) {
            if (entityColumnsMap.containsKey(entityType.getName())) {
                hashMap = entityColumnsMap.get(entityType.getName());
            } else {
    ...
    public static Column getColumnOrId(Class<?> entityType, String columnName) {
        return getPrimaryKeyColumnName(entityType).equals(columnName) ? getId(entityType) : getColumnMap(entityType).get(columnName);
    }
    ...
    public static synchronized Id getId(Class<?> entityType) {
        ...
        synchronized (TableUtils.class) {
            ...
            } else if (entityIdMap.containsKey(entityType.getName())) {
                id = entityIdMap.get(entityType.getName());
            } else {
                ...
                if (fields != null) {
                    ...
                    if (primaryKeyField == null) {
                        for (Field field2 : fields) {
                            if ("id".equals(field2.getName()) || com.yolanda.nohttp.db.Field.ID.equals(field2.getName())) {
