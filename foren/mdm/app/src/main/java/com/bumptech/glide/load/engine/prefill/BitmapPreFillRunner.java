    ...
    private boolean allocate() {
        ...
        while (!this.toPrefill.isEmpty() && !isGcDetected(start)) {
            ...
            if (Log.isLoggable(TAG, 3)) {
                Log.d(TAG, "allocated [" + toAllocate.getWidth() + EMPrivateConstant.EMMultiUserConstant.MUC_ELEMENT_NAME + toAllocate.getHeight() + "] " + toAllocate.getConfig() + " size: " + Util.getBitmapByteSize(bitmap));
            }
        }
