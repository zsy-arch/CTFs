...
public final class TransformationUtils {
    ...
    public static Bitmap fitCenter(Bitmap toFit, BitmapPool pool, int width, int height) {
        if (toFit.getWidth() != width || toFit.getHeight() != height) {
            float minPercentage = Math.min(width / toFit.getWidth(), height / toFit.getHeight());
            ...
            if (toFit.getWidth() != targetWidth || toFit.getHeight() != targetHeight) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    Log.v(TAG, "request: " + width + EMPrivateConstant.EMMultiUserConstant.MUC_ELEMENT_NAME + height);
                    Log.v(TAG, "toFit:   " + toFit.getWidth() + EMPrivateConstant.EMMultiUserConstant.MUC_ELEMENT_NAME + toFit.getHeight());
                    Log.v(TAG, "toReuse: " + toReuse.getWidth() + EMPrivateConstant.EMMultiUserConstant.MUC_ELEMENT_NAME + toReuse.getHeight());
