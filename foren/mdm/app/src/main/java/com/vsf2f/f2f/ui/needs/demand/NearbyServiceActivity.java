...
import com.vsf2f.f2f.adapter.NearbyServiceAdapter;
...
public class NearbyServiceActivity extends BaseActivity implements AMap.OnMapClickListener, AMap.OnMyLocationChangeListener, AMap.OnMarkerClickListener, AMap.OnCameraChangeListener, NearbyServiceAdapter.AdapterItemClick {
    ...
    public void initView() {
        initHeaderBack(R.string.nearby_task_service, 0);
    ...
    public void initData() {
        ...
        this.descriptionDialog.setOnDismissListener(new DialogInterface.OnDismissListener() { // from class: com.vsf2f.f2f.ui.needs.demand.NearbyServiceActivity.1
            @Override // android.content.DialogInterface.OnDismissListener
            public void onDismiss(DialogInterface dialog) {
                if (NearbyServiceActivity.this.clickMarker != null) {
                    AmapUtils.getBitmap(NearbyServiceActivity.this.markerIndex, false);
                }
                NearbyServiceActivity.this.clickMarker = null;
            }
        });
        this.adapter = new NearbyServiceAdapter(this.dataList, this);
    ...
    private void initBehavior() {
        ...
        this.behavior.setBottomSheetCallback(new BottomSheetBehavior.BottomSheetCallback() { // from class: com.vsf2f.f2f.ui.needs.demand.NearbyServiceActivity.2
            @Override // android.support.design.widget.BottomSheetBehavior.BottomSheetCallback
            public void onStateChanged(@NonNull View bottomSheet, int newState) {
                switch (newState) {
                    case 3:
                        NearbyServiceActivity.this.ll_location_map.setVisibility(4);
                        return;
                    case 4:
                        NearbyServiceActivity.this.ll_location_map.setVisibility(4);
                        NearbyServiceActivity.this.ll_location.setVisibility(0);
                        return;
                    case 5:
                        NearbyServiceActivity.this.ll_location_map.setVisibility(0);
                        return;
                    default:
                        return;
                }
            ...
            public void onSlide(@NonNull View bottomSheet, float slideOffset) {
                if (slideOffset > 0.5d) {
                    NearbyServiceActivity.this.ll_location.setVisibility(8);
                }
            }
        });
    }
    ...
    public void expandDetail(View view) {
        if (this.dataList.size() == 0) {
            MyToast.show(getApplicationContext(), "当前附近没有服务,稍后再试");
            return;
        }
    ...
    public void publish(View view) {
        if (isNoLogin()) {
            ...
            startAct(ServicePublishActivity.class, bundle1);
        }
    }
    ...
    @Override // com.vsf2f.f2f.adapter.NearbyServiceAdapter.AdapterItemClick
    ...
    public void getOrder(DemandDetailBean bean) {
        if (isNoLogin()) {
            ...
            String sername = bean.getPublishUserObj().getUserName();
            if (TextUtils.isEmpty(sername)) {
                return;
            }
            if (cusname.equals(sername)) {
                MyToast.show(getApp(), "不能预约自己的服务");
                return;
            }
            Bundle bundle2 = new Bundle();
            bundle2.putInt("id", bean.getMoId());
    ...
    private void getNearbyService() {
        getClient().showDialogNow();
        AjaxParams params = new AjaxParams();
        getClient().get(R.string.API_SERVICE_HOMEPAGE, ComUtil.getXDDApi(this.context, getString(R.string.API_SERVICE_HOMEPAGE)) + "?pageNo=" + this.pageNo + "&pageSize=10&lat=" + this.latitude + "&lng=" + this.longitude + "&zoom=" + this.currentZoom + "&orderBy=2", params, DemandServiceBean.class);
    }
