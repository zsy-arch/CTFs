...
public class Needs2Activity extends BaseActivity implements NavGroup.OnCheckedChangeListener, XRefreshViewListener, DemandAdapter.AdapterItemClick, DemandBrushDialog.BrushType, AMapLocationListener, XScrollView.OnScrollListener, BasePopupMenu.PopupListener {
    ...
    private ImageCycleView.ImageCycleViewListener mAdCycleViewListener = new ImageCycleView.ImageCycleViewListener() { // from class: com.vsf2f.f2f.fragment.Needs2Activity.2
        @Override // com.vsf2f.f2f.ui.view.ImageCycleView.ImageCycleViewListener
        public void onImageClick(int position, View imageView) {
            switch (position) {
                case 0:
                    Needs2Activity.this.startToHelp();
                    return;
                case 1:
                    if (UserShared.getInstance().getInt(Constant.CERT_MOBILE) != 1) {
                        new EaseAlertDialog(Needs2Activity.this.context, (int) R.string.prompt, (int) R.string.not_bing_phone_prompt, (Bundle) null, new EaseAlertDialog.AlertDialogListener() { // from class: com.vsf2f.f2f.fragment.Needs2Activity.2.1
                            @Override // com.easeui.widget.EaseAlertDialog.AlertDialogListener
                            public void onResult(boolean confirmed, Bundle bundle) {
                                if (confirmed) {
                                    Needs2Activity.this.startAct(BindPhoneActivity.class);
                                }
                            }
                        }, true).show();
                        return;
                    } else if (UserShared.getInstance().isOpenManor()) {
                        GameUtil.startGame(Needs2Activity.this.context);
                        return;
                    } else {
                        new ManorTreeDialog(Needs2Activity.this.context).show();
                        return;
                    }
                default:
                    try {
                        String url = ((UserShareBean) Needs2Activity.this.shareList.get(position - 2)).getShareHref();
                        Bundle bundlePre = new Bundle();
                        bundlePre.putString(com.hy.frame.util.Constant.FLAG, WebUtils.getTokenUrl(Needs2Activity.this.context, url));
                        bundlePre.putBoolean(com.hy.frame.util.Constant.FLAG2, true);
                        Needs2Activity.this.startAct(WebKitLocalActivity.class, bundlePre);
                        return;
    ...
    public int initLayoutId() {
        return R.layout.act_service_main;
    }
    ...
    public void initView() {
        ...
        this.adapter = new DemandAdapter(this.serviceList, this);
        ...
        if (isLogin()) {
            ...
            fly_list_myorder.postDelayed(new Runnable() { // from class: com.vsf2f.f2f.fragment.Needs2Activity.1
                @Override // java.lang.Runnable
                public void run() {
                    fly_list_myorder.setVisibility(0);
                    fly_list_myorder.setAnimation(AnimationUtils.makeInAnimation(Needs2Activity.this.context, false));
                }
            }, 3000L);
        }
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.discover_imgTree /* 2131755301 */:
                if (isNoLogin()) {
                    ...
                    new EaseAlertDialog(this.context, (int) R.string.prompt, (int) R.string.not_bing_phone_prompt, (Bundle) null, new EaseAlertDialog.AlertDialogListener() { // from class: com.vsf2f.f2f.fragment.Needs2Activity.3
                        @Override // com.easeui.widget.EaseAlertDialog.AlertDialogListener
    ...
    public void startToHelp() {
        Bundle bundle = new Bundle();
        String sph_url = DemoHelper.getInstance().readConfig().getSph_url();
        if (TextUtils.isEmpty(sph_url)) {
            sph_url = getString(R.string.API_SERVICE_HELP);
        }
        bundle.putString(com.hy.frame.util.Constant.FLAG, sph_url);
    ...
    private void getServiceList() {
        ...
        getClient().get(R.string.API_SERVICE_HOMEPAGE, ComUtil.getXDDApi(this.context, getString(R.string.API_SERVICE_HOMEPAGE)) + "?pageNo=" + this.page + "&pageSize=20&lat=" + this.latitude + "&lng=" + this.longitude + "&distance=1000&orderBy=" + this.orderType + "&serviceMode=" + this.serviceType, params, DemandServiceBean.class);
    }
    ...
    public void itemClick(int position) {
        ...
        if (HyUtil.isNoEmpty(this.serviceList) && position < this.serviceList.size()) {
            bean = this.serviceList.get(position);
        }
        if (bean != null) {
    ...
    public void onScroll(int l, final int t, int oldl, int oldt) {
        ThreadPool.newThreadPool(new Runnable() { // from class: com.vsf2f.f2f.fragment.Needs2Activity.4
            @Override // java.lang.Runnable
            public void run() {
                if (Needs2Activity.this.showTopScreen && t < Needs2Activity.this.headerHeight) {
                    Needs2Activity.this.showTopScreen = false;
                    Needs2Activity.this.topScreen.post(new Runnable() { // from class: com.vsf2f.f2f.fragment.Needs2Activity.4.1
                        @Override // java.lang.Runnable
                        public void run() {
                            Needs2Activity.this.topScreen.setVisibility(8);
                        }
                    });
                    MyLog.e("topScreen:GONE");
                } else if (!Needs2Activity.this.showTopScreen && t > Needs2Activity.this.headerHeight) {
                    Needs2Activity.this.showTopScreen = true;
                    Needs2Activity.this.topScreen.post(new Runnable() { // from class: com.vsf2f.f2f.fragment.Needs2Activity.4.2
                        @Override // java.lang.Runnable
                        public void run() {
                            Needs2Activity.this.topScreen.setVisibility(0);
                        }
                    });
