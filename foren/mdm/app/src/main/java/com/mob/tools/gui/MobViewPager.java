...
public class MobViewPager extends ViewGroup {
    ...
    private void handleInterceptMove(MotionEvent ev) {
        ...
        if (((int) Math.abs(y - this.lastMotionY)) < xDiff && xDiff > this.touchSlop) {
            this.touchState = 1;
            this.lastMotionX = x;
        }
    }
    ...
    private void handleScrollMove(MotionEvent ev) {
        ...
        if (this.adapter != null) {
            ...
            } else if (deltaX > 0 && getChildCount() != 0 && (availableToScroll = (getChildAt(getChildCount() - 1).getRight() - getScrollX()) - getWidth()) > 0) {
    ...
    private void onScreenChange(int lastScreen) {
        if (this.adapter != null) {
            if (this.currentScreen > lastScreen) {
                ...
                for (int i2 = 0; i2 < lastScreen - this.currentScreen; i2++) {
                    int screen2 = (lastScreen - i2) - 1;
                    View tmp2 = this.nextPage;
                    ...
                    if (screen2 > 0) {
                        this.previousPage = this.adapter.getView(screen2 - 1, tmp2, this);
                        addView(this.previousPage, 0);
                    } else {
                        this.previousPage = tmp2;
                    }
                }
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.adapter != null && this.screenCount > 0) {
            ...
            int adjustedWidthMeasureSpec = View.MeasureSpec.makeMeasureSpec(width, 1073741824);
            int adjustedHeightMeasureSpec = View.MeasureSpec.makeMeasureSpec(height, 1073741824);
