...
public class GcmRegistrar {
    ...
    public Task<Void> registerAsync() {
        ...
        synchronized (this.lock) {
            ...
            onSuccessTask = (installation.getDeviceToken() == null ? Task.forResult(true) : isLocalDeviceTokenStaleAsync()).onSuccessTask(new Continuation<Boolean, Task<Void>>() { // from class: com.parse.GcmRegistrar.1
                ...
                synchronized (GcmRegistrar.this.localDeviceTokenLastModifiedMutex) {
                    if (GcmRegistrar.this.localDeviceTokenLastModified == 0) {
                        try {
                            ...
                            GcmRegistrar.this.localDeviceTokenLastModified = Long.valueOf(localDeviceTokenLastModifiedStr).longValue();
                        } catch (IOException e) {
    ...
    public static class Request {
        ...
        private final BroadcastReceiver retryReceiver = new BroadcastReceiver() { // from class: com.parse.GcmRegistrar.Request.1
        ...
        public void onReceiveResponseIntent(Intent intent) {
            ...
            if (registrationId == null && error == null) {
                ...
                ((AlarmManager) this.context.getSystemService(NotificationCompat.CATEGORY_ALARM)).set(2, SystemClock.elapsedRealtime() + ((1 << this.tries.get()) * 3000) + this.random.nextInt(3000), this.retryIntent);
            }
        }
