...
public class ParseQuery<T extends ParseObject> {
    ...
    public static class State<T extends ParseObject> {
        ...
        public static class Builder<T extends ParseObject> {
            ...
            public static <T extends ParseObject> Builder<T> or(List<Builder<T>> builders) {
                ...
                for (Builder<T> builder : builders) {
                    if (className != null && !((Builder) builder).className.equals(className)) {
                        throw new IllegalArgumentException("All of the queries in an or query must be on the same class ");
                    } else if (((Builder) builder).limit >= 0) {
                        throw new IllegalArgumentException("Cannot have limits in sub queries of an 'OR' query");
                    } else if (((Builder) builder).skip > 0) {
                        throw new IllegalArgumentException("Cannot have skips in sub queries of an 'OR' query");
                    } else if (!((Builder) builder).order.isEmpty()) {
                        throw new IllegalArgumentException("Cannot have an order in sub queries of an 'OR' query");
                    } else if (!((Builder) builder).includes.isEmpty()) {
                        throw new IllegalArgumentException("Cannot have an include in sub queries of an 'OR' query");
                    } else if (((Builder) builder).selectedKeys != null) {
                        throw new IllegalArgumentException("Cannot have an selectKeys in sub queries of an 'OR' query");
                    } else {
                        className = ((Builder) builder).className;
                        constraints.add(((Builder) builder).where);
                    }
                }
            ...
            public Builder(Class<T> subclass) {
                this(ParseQuery.getSubclassingController().getClassName(subclass));
            }
        ...
        private State(Builder<T> builder) {
            ...
            if (((Builder) builder).selectedKeys != null) {
                set = Collections.unmodifiableSet(new HashSet(((Builder) builder).selectedKeys));
            } else {
                set = null;
            }
    ...
    public T getFirst() throws ParseException {
        return (T) ((ParseObject) ParseTaskUtils.wait(getFirstInBackground()));
    }
    ...
    private <TResult> Task<TResult> doWithRunningCheck(Callable<Task<TResult>> runnable) {
        ...
        return (Task<TResult>) task.continueWithTask((Continuation<TResult, Task<TResult>>) new Continuation<TResult, Task<TResult>>() { // from class: com.parse.ParseQuery.1
            /* JADX WARN: Unknown type variable: TResult in type: bolts.Task<TResult> */
    ...
    public void findInBackground(FindCallback<T> callback) {
        ...
        if (state.cachePolicy() != CachePolicy.CACHE_THEN_NETWORK || state.isFromLocalDatastore()) {
            ...
            task = doCacheThenNetwork(state, callback, new CacheThenNetworkCallable<T, Task<List<T>>>() { // from class: com.parse.ParseQuery.2
                @Override // com.parse.ParseQuery.CacheThenNetworkCallable
                public Task<List<T>> call(State<T> state2, ParseUser user, Task<Void> cancellationToken) {
                    return ParseQuery.this.findAsync(state2, user, cancellationToken);
                }
            });
        }
    ...
    private Task<List<T>> findAsync(final State<T> state) {
        return (Task<List<T>>) doWithRunningCheck(new Callable<Task<List<T>>>() { // from class: com.parse.ParseQuery.3
            @Override // java.util.concurrent.Callable
            public Task<List<T>> call() throws Exception {
                return (Task<List<T>>) ParseQuery.this.getUserAsync(state).onSuccessTask(new Continuation<ParseUser, Task<List<T>>>() { // from class: com.parse.ParseQuery.3.1
                    @Override // bolts.Continuation
    ...
    public void getFirstInBackground(GetCallback<T> callback) {
        ...
        if (state.cachePolicy() != CachePolicy.CACHE_THEN_NETWORK || state.isFromLocalDatastore()) {
            ...
            task = doCacheThenNetwork(state, callback, new CacheThenNetworkCallable<T, Task<T>>() { // from class: com.parse.ParseQuery.4
                @Override // com.parse.ParseQuery.CacheThenNetworkCallable
                public Task<T> call(State<T> state2, ParseUser user, Task<Void> cancellationToken) {
                    return ParseQuery.this.getFirstAsync(state2, user, cancellationToken);
                }
            });
        }
        ParseTaskUtils.callbackOnMainThreadAsync(task, callback);
    ...
    private Task<T> getFirstAsync(final State<T> state) {
        return (Task<T>) doWithRunningCheck(new Callable<Task<T>>() { // from class: com.parse.ParseQuery.5
            @Override // java.util.concurrent.Callable
            public Task<T> call() throws Exception {
                return (Task<T>) ParseQuery.this.getUserAsync(state).onSuccessTask(new Continuation<ParseUser, Task<T>>() { // from class: com.parse.ParseQuery.5.1
                    @Override // bolts.Continuation
    ...
    public Task<T> getFirstAsync(State<T> state, ParseUser user, Task<Void> cancellationToken) {
    ...
    public void countInBackground(final CountCallback callback) {
        Task task;
        State<T> state = new State.Builder<>(this.builder).setLimit(0).build();
        ParseCallback2 parseCallback2 = callback != null ? new ParseCallback2<Integer, ParseException>() { // from class: com.parse.ParseQuery.6
        ...
        if (state.cachePolicy() != CachePolicy.CACHE_THEN_NETWORK || state.isFromLocalDatastore()) {
            ...
            task = doCacheThenNetwork(state, parseCallback2, new CacheThenNetworkCallable<T, Task<Integer>>() { // from class: com.parse.ParseQuery.7
                @Override // com.parse.ParseQuery.CacheThenNetworkCallable
                public Task<Integer> call(State<T> state2, ParseUser user, Task<Void> cancellationToken) {
                    return ParseQuery.this.countAsync(state2, user, cancellationToken);
                }
            });
        }
        ParseTaskUtils.callbackOnMainThreadAsync(task, parseCallback2);
    }

    private Task<Integer> countAsync(final State<T> state) {
        return doWithRunningCheck(new Callable<Task<Integer>>() { // from class: com.parse.ParseQuery.8
            @Override // java.util.concurrent.Callable
            public Task<Integer> call() throws Exception {
                return ParseQuery.this.getUserAsync(state).onSuccessTask(new Continuation<ParseUser, Task<Integer>>() { // from class: com.parse.ParseQuery.8.1
                    @Override // bolts.Continuation
    ...
    public Task<Integer> countAsync(State<T> state, ParseUser user, Task<Void> cancellationToken) {
        return getQueryController().countAsync(state, user, cancellationToken);
    }

    public T get(String objectId) throws ParseException {
        return (T) ((ParseObject) ParseTaskUtils.wait(getInBackground(objectId)));
    }
    ...
    public boolean hasCachedResult() {
        ...
        return ParseKeyValueCache.loadFromKeyValueCache(ParseRESTQueryCommand.findCommand(state, user != null ? user.getSessionToken() : null).getCacheKey(), state.maxCacheAge()) != null;
    }
    ...
    public void getInBackground(String objectId, GetCallback<T> callback) {
        ...
        if (state.cachePolicy() != CachePolicy.CACHE_THEN_NETWORK || state.isFromLocalDatastore()) {
            ...
            task = doCacheThenNetwork(state, callback, new CacheThenNetworkCallable<T, Task<T>>() { // from class: com.parse.ParseQuery.9
                @Override // com.parse.ParseQuery.CacheThenNetworkCallable
                public Task<T> call(State<T> state2, ParseUser user, Task<Void> cancellationToken) {
                    return ParseQuery.this.getFirstAsync(state2, user, cancellationToken);
                }
    ...
    /* renamed from: com.parse.ParseQuery$10 */
    /* loaded from: classes2.dex */
    public class AnonymousClass10 implements Callable<Task<TResult>> {
        ...
        public Task<TResult> call() throws Exception {
            return ParseQuery.this.getUserAsync(this.val$state).onSuccessTask(new Continuation<ParseUser, Task<TResult>>() { // from class: com.parse.ParseQuery.10.1
                /* JADX WARN: Unknown type variable: TResult in type: bolts.Task<TResult> */
                @Override // bolts.Continuation
                public Task<TResult> then(Task<ParseUser> task) throws Exception {
                    final ParseUser user = task.getResult();
                    State<T> cacheState = new State.Builder(AnonymousClass10.this.val$state).setCachePolicy(CachePolicy.CACHE_ONLY).build();
                    final State<T> networkState = new State.Builder(AnonymousClass10.this.val$state).setCachePolicy(CachePolicy.NETWORK_ONLY).build();
                    return ParseTaskUtils.callbackOnMainThreadAsync((Task) AnonymousClass10.this.val$delegate.call(cacheState, user, ParseQuery.this.cts.getTask()), AnonymousClass10.this.val$callback).continueWithTask(new Continuation<TResult, Task<TResult>>() { // from class: com.parse.ParseQuery.10.1.1
                        /* JADX WARN: Unknown type variable: TResult in type: bolts.Task<TResult> */
                        @Override // bolts.Continuation
                        public Task<TResult> then(Task<TResult> task2) throws Exception {
                            ...
                            return (Task) AnonymousClass10.this.val$delegate.call(networkState, user, ParseQuery.this.cts.getTask());
                        }
                    });
                }
            });
    ...
    private <TResult> Task<TResult> doCacheThenNetwork(State<T> state, ParseCallback2<TResult, ParseException> callback, CacheThenNetworkCallable<T, Task<TResult>> delegate) {
        return doWithRunningCheck(new AnonymousClass10(state, delegate, callback));
    }
    ...
    public ParseQuery<T> whereContainsAll(String key, Collection<?> values) {
        checkIfRunning();
        this.builder.addCondition(key, "$all", (Collection<? extends Object>) values);
        return this;
    }
    ...
    public ParseQuery<T> whereExists(String key) {
        checkIfRunning();
        this.builder.addCondition(key, "$exists", (Object) true);
        return this;
    }

    public ParseQuery<T> whereDoesNotExist(String key) {
        checkIfRunning();
        this.builder.addCondition(key, "$exists", (Object) false);
        return this;
    }
