...
public abstract class ParseEventuallyQueue {
    ...
    public void notifyTestHelper(int event) {
    ...
    public static class TestHelper {
        ...
        public int unexpectedEvents() {
            int sum = 0;
            for (int i = 0; i < this.events.size(); i++) {
                sum += this.events.get(this.events.keyAt(i)).availablePermits();
            }
            return sum;
        }
        ...
        public List<String> getUnexpectedEvents() {
            ...
            for (int i = 0; i < this.events.size(); i++) {
                ...
                if (this.events.get(event).availablePermits() > 0) {
        ...
        public boolean waitFor(int event, int permits) {
            try {
                return this.events.get(event).tryAcquire(permits, 10L, TimeUnit.SECONDS);
