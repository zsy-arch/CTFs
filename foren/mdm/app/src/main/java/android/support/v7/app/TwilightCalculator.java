...
class TwilightCalculator {
    ...
    public void calculateTwilight(long time, double latitude, double longitude) {
        ...
        double solarLng = 1.796593063d + meanAnomaly + (0.03341960161924362d * Math.sin(meanAnomaly)) + (3.4906598739326E-4d * Math.sin(2.0f * meanAnomaly)) + (5.236000106378924E-6d * Math.sin(3.0f * meanAnomaly)) + 3.141592653589793d;
        double arcLongitude = (-longitude) / 360.0d;
        double solarTransitJ2000 = J0 + ((float) Math.round((daysSince2000 - J0) - arcLongitude)) + arcLongitude + (0.0053d * Math.sin(meanAnomaly)) + ((-0.0069d) * Math.sin(2.0d * solarLng));
