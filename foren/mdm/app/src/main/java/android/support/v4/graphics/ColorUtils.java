...
public final class ColorUtils {
    ...
    public static int calculateMinimumAlpha(@ColorInt int foreground, @ColorInt int background, float minContrastRatio) {
        if (Color.alpha(background) != 255) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(background));
        } else if (calculateContrast(setAlphaComponent(foreground, 255), background) < minContrastRatio) {
            return -1;
        } else {
            int minAlpha = 0;
            int maxAlpha = 255;
            for (int numIterations = 0; numIterations <= 10 && maxAlpha - minAlpha > 1; numIterations++) {
                ...
                if (calculateContrast(setAlphaComponent(foreground, testAlpha), background) < minContrastRatio) {
                    minAlpha = testAlpha;
                } else {
                    maxAlpha = testAlpha;
                }
    ...
    public static int blendARGB(@ColorInt int color1, @ColorInt int color2, @FloatRange(from = 0.0d, to = 1.0d) float ratio) {
        ...
        return Color.argb((int) ((Color.alpha(color1) * inverseRatio) + (Color.alpha(color2) * ratio)), (int) ((Color.red(color1) * inverseRatio) + (Color.red(color2) * ratio)), (int) ((Color.green(color1) * inverseRatio) + (Color.green(color2) * ratio)), (int) ((Color.blue(color1) * inverseRatio) + (Color.blue(color2) * ratio)));
    }
