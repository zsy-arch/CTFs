...
public class ViewPager extends ViewGroup {
    ...
    private void scrollToItem(int item, boolean smoothScroll, int velocity, boolean dispatchSelected) {
        ...
        int destX = 0;
        if (curInfo != null) {
            destX = (int) (getClientWidth() * Math.max(this.mFirstOffset, Math.min(curInfo.offset, this.mLastOffset)));
        }
        if (smoothScroll) {
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = halfWidth + (halfWidth * distanceInfluenceForSnapDuration(Math.min(1.0f, (1.0f * Math.abs(dx)) / width)));
        int velocity2 = Math.abs(velocity);
        if (velocity2 > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / velocity2)) * 4;
        } else {
            duration = (int) ((1.0f + (Math.abs(dx) / (this.mPageMargin + (width * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i2 = 0; i2 < size2; i2++) {
            ...
            if (child2.getVisibility() != 8 && ((lp = (LayoutParams) child2.getLayoutParams()) == null || !lp.isDecor)) {
                child2.measure(View.MeasureSpec.makeMeasureSpec((int) (childWidthSize * lp.widthFactor), 1073741824), this.mChildHeightMeasureSpec);
            }
        }
    }
    ...
    private void recomputeScrollPosition(int width, int oldWidth, int margin, int oldMargin) {
        if (oldWidth <= 0 || this.mItems.isEmpty()) {
            ...
            int scrollPos = (int) (((width - getPaddingLeft()) - getPaddingRight()) * (ii != null ? Math.min(ii.offset, this.mLastOffset) : 0.0f));
        ...
        for (int i2 = 0; i2 < count; i2++) {
            ...
            if (child2.getVisibility() != 8) {
                ...
                if (!lp2.isDecor && (ii = infoForChild(child2)) != null) {
                    int childLeft3 = paddingLeft + ((int) (childWidth * ii.offset));
                    if (lp2.needsMeasure) {
                        lp2.needsMeasure = false;
                        child2.measure(View.MeasureSpec.makeMeasureSpec((int) (childWidth * lp2.widthFactor), 1073741824), View.MeasureSpec.makeMeasureSpec((height - paddingTop) - paddingBottom, 1073741824));
                    }
        ...
        if (this.mPageTransformer != null) {
            ...
            for (int i2 = 0; i2 < childCount2; i2++) {
                ...
                if (!((LayoutParams) child2.getLayoutParams()).isDecor) {
                    this.mPageTransformer.transformPage(child2, (child2.getLeft() - scrollX2) / getClientWidth());
                }
            }
        }
        ...
        switch (ev.getAction() & 255) {
            ...
            case 1:
                if (this.mIsBeingDragged) {
                    ...
                    int scrollX = getScrollX();
                    ItemInfo ii = infoForCurrentScrollPosition();
                    setCurrentItemInternal(determineTargetPage(ii.position, ((scrollX / width) - ii.offset) / (ii.widthFactor + (this.mPageMargin / width)), initialVelocity, (int) (ev.getX(ev.findPointerIndex(this.mActivePointerId)) - this.mInitialMotionX)), true, true, initialVelocity);
    ...
    private boolean performDrag(float x) {
        ...
        if (scrollX < leftBound) {
            if (leftAbsolute) {
                this.mLeftEdge.onPull(Math.abs(leftBound - scrollX) / width);
                needsInvalidate = true;
            }
            scrollX = leftBound;
        } else if (scrollX > rightBound) {
            if (rightAbsolute) {
                this.mRightEdge.onPull(Math.abs(scrollX - rightBound) / width);
                needsInvalidate = true;
            }
            scrollX = rightBound;
        }
    ...
    public void endFakeDrag() {
        ...
        if (this.mAdapter != null) {
            ...
            int scrollX = getScrollX();
            ItemInfo ii = infoForCurrentScrollPosition();
            setCurrentItemInternal(determineTargetPage(ii.position, ((scrollX / width) - ii.offset) / ii.widthFactor, initialVelocity, (int) (this.mLastMotionX - this.mInitialMotionX)), true, true, initialVelocity);
        }
        endDrag();
