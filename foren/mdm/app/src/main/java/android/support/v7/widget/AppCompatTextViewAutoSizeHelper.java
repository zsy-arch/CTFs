...
public class AppCompatTextViewAutoSizeHelper {
        ...
        if (isAutoSizeEnabled()) {
            if (this.mNeedsAutoSizeText) {
                if (this.mTextView.getMeasuredHeight() > 0 && this.mTextView.getMeasuredWidth() > 0) {
                    int availableWidth = ((Boolean) invokeAndReturnWithDefault(this.mTextView, "getHorizontallyScrolling", false)).booleanValue() ? 1048576 : (this.mTextView.getMeasuredWidth() - this.mTextView.getTotalPaddingLeft()) - this.mTextView.getTotalPaddingRight();
    ...
    private <T> T invokeAndReturnWithDefault(@NonNull Object object, @NonNull String methodName, @NonNull T defaultValue) {
        ...
        try {
            result = null;
            try {
                T result2 = (T) getTextViewMethod(methodName).invoke(object, new Object[0]);
                if (result2 != null || 0 == 0) {
                    return result2;
                }
                result = defaultValue;
