...
public class ChangeImageTransform extends Transition {
    ...
    private static Matrix centerCropMatrix(ImageView view) {
        ...
        float maxScale = Math.max(imageViewWidth / imageWidth, imageViewHeight / imageHeight);
        int tx = Math.round((imageViewWidth - (imageWidth * maxScale)) / 2.0f);
        int ty = Math.round((imageViewHeight - (imageHeight * maxScale)) / 2.0f);
        Matrix matrix = new Matrix();
