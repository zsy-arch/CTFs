    ...
    void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            if (!this.mRunning) {
                ...
                float clearRadius = (width / 2) * INTERPOLATOR.getInterpolation((((float) ((now - this.mFinishTime) % 1000)) / 10.0f) / 100.0f);
            ...
            if (rawProgress >= 0.0f && rawProgress <= 25.0f) {
                drawCircle(canvas, cx, cy, this.mColor1, ((25.0f + rawProgress) * 2.0f) / 100.0f);
            }
            if (rawProgress >= 0.0f && rawProgress <= 50.0f) {
                drawCircle(canvas, cx, cy, this.mColor2, (2.0f * rawProgress) / 100.0f);
            }
            if (rawProgress >= 25.0f && rawProgress <= 75.0f) {
                drawCircle(canvas, cx, cy, this.mColor3, ((rawProgress - 25.0f) * 2.0f) / 100.0f);
            }
            if (rawProgress >= 50.0f && rawProgress <= 100.0f) {
                drawCircle(canvas, cx, cy, this.mColor4, ((rawProgress - 50.0f) * 2.0f) / 100.0f);
            }
            if (rawProgress >= 75.0f && rawProgress <= 100.0f) {
                drawCircle(canvas, cx, cy, this.mColor1, ((rawProgress - 75.0f) * 2.0f) / 100.0f);
            }
