        ...
        if (removalsPending || movesPending || additionsPending || changesPending) {
            ...
            if (additionsPending) {
                ...
                if (removalsPending || movesPending || changesPending) {
                    ViewCompat.postOnAnimationDelayed(additions.get(0).itemView, adder, (removalsPending ? getRemoveDuration() : 0L) + Math.max(movesPending ? getMoveDuration() : 0L, changesPending ? getChangeDuration() : 0L));
                } else {
                    adder.run();
                }
            }
