...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView {
    ...
    private boolean canScroll() {
        ...
        return getHeight() < (getPaddingTop() + child.getHeight()) + getPaddingBottom();
    }
    ...
    protected int computeScrollDeltaToGetChildRectOnScreen(Rect rect) {
        ...
        if (rect.bottom > screenBottom && rect.top > screenTop) {
            return Math.min(rect.height() > height ? 0 + (rect.top - screenTop) : 0 + (rect.bottom - screenBottom), getChildAt(0).getBottom() - screenBottom);
        } else if (rect.top >= screenTop || rect.bottom >= screenBottom) {
            return 0;
        } else {
            return Math.max(rect.height() > height ? 0 - (screenBottom - rect.bottom) : 0 - (screenTop - rect.top), -getScrollY());
        }
    }
