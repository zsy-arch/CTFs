...
public class ViewDragHelper {
    ...
    private int computeSettleDuration(View child, int dx, int dy, int xvel, int yvel) {
        ...
        return (int) ((computeAxisDuration(dx, xvel2, this.mCallback.getViewHorizontalDragRange(child)) * (xvel2 != 0 ? absXVel / addedVel : absDx / addedDistance)) + (computeAxisDuration(dy, yvel2, this.mCallback.getViewVerticalDragRange(child)) * (yvel2 != 0 ? absYVel / addedVel : absDy / addedDistance)));
    }
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        float distance = halfWidth + (halfWidth * distanceInfluenceForSnapDuration(Math.min(1.0f, Math.abs(delta) / width)));
