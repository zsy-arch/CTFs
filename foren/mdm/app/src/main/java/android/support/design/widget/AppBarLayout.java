    ...
    public static class Behavior extends HeaderBehavior<AppBarLayout> {
        ...
        private void animateOffsetTo(CoordinatorLayout coordinatorLayout, AppBarLayout child, int offset, float velocity) {
            ...
            if (velocity2 > 0.0f) {
                ...
                duration = (int) ((1.0f + (distance / child.getHeight())) * 150.0f);
            }
            ...
            for (int i = 0; i < z; i++) {
                ...
                if (absOffset >= child.getTop() && absOffset <= child.getBottom()) {
                    if (interpolator == null) {
                        ...
                        return Integer.signum(offset) * (child.getTop() + Math.round(childScrollableHeight * interpolator.getInterpolation((absOffset - child.getTop()) / childScrollableHeight)));
                    }
                }
            }
            return offset;
